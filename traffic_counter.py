...
class TrafficCounter(object):
    def __init__(self,video_source=0,
        ...
        self.prev_ids    = []          #this will contain the coordinates of the centers in the previos
    ...
    def _draw_bounding_boxes(self,frame,contour_id,bounding_points,cx,cy,prev_cx,prev_cy):
        ...
        cv2.putText(frame,str(contour_id),(cx,cy-15),self.font,0.4,(255,0,0),2) #this line assigns ID's to the contours in each frame
    ...
    def bind_objects(self,frame,thresh_img):
        ...
        '''
        datetime_object = datetime.datetime.now()   #Daniel
        ...
        for c in cnts:
            ...
            cur_ids.append(cnt_id)  # save ids
            ...
            if len(self.prev_centroids)==0: 
                ...
                minPoint = None
                minDist = None
                for i,j in zip(self.prev_centroids,self.prev_ids):
                    dist = np.linalg.norm(C - i)                #numpy's way to find the euclidean distance between two points
            ...
            print(f"Area of Vehicle: {cv2.contourArea(c)}")                                    #Daniel
            #print(f"Current id: {cnt_id}")                                                    #Daniel
            #print(f"Previous id: {prev_id}")                                                  #Daniel
            print(f"Current coordinates: {cx}, {cy}")                                          #Daniel
            print(f"Previous coordinates: {prev_cx}, {prev_cy}")                               #Daniel
            print(f"Line Length: {(cx-prev_cx), (cy-prev_cy) }")     #Daniel
            print(f"---")     
            
            #print(f"{cv2.contourArea(c)}")                                                    #Daniel
           
            
            _is_crossed = self._is_line_crossed(frame,cx,cy,prev_cx,prev_cy)
            if _is_crossed:
                print(f"________________________________________")              #Daniel
                print(f"Date & Time: {datetime_object}")                        #Daniel
                print(f"Total Count: {self.counter}")                   
                print(f"Vehicle Number (ID): {cnt_id}")                         #Daniel
                print(f"Area of Vehicle: {cv2.contourArea(c)}")                 #Daniel
                print(f"Length of Vehicle: {h}")                                #Daniel
                print(f"Width of Vehicle: {w}")                                 #Daniel
                print(f"Vehicle Type: {(cv2.contourArea(c)**2)/10000}")         #Daniel
                print(f"Current x-coord: {cx}")                                 #Daniel
                print(f"Current y-coord: {cy}")                                 #Daniel
                print(f"Previous x-coord: {prev_cx}")                           #Daniel
                print(f"Previous y-coord: {prev_cy}")                           #Daniel
            ...
            ##sheet['B1'] = "x-coord"
            ##sheet['C1'] = "y-coord"
            ##sheet['D1'] = "Vehicle Type"

            ##sheet['A2'] = "x"
            ##sheet['B2'] = cx
            ##sheet['C2'] = cy
            ##sheet['D2'] = (cv2.contourArea(c)**2)/10000
        ...
        self.prev_ids = cur_ids                     #updating centroids for next frame
        #print(f"Current id: {cnt_id}")                                                  #Daniel
        #print(f"Previous id: {prev_id}")                                                #Daniel
    ...
    def main_loop(self):
        ...
        from timeit import default_timer as timer           #Daniel
        from datetime import timedelta                      #Daniel
        ...
        while True:
            ...
            print(f"________________________________________")                          #Daniel
            print(f"Frame Number: {frame_id}")                                          #Daniel
            print(f"Vehicle ID: {1 + self.counter}")                                    #Daniel
            #print(f"Centroid x-coord: {int(cv2.minAreaRect(c)[0][0])}")                #Daniel
            #print(f"Centroid y-coord: {int(cv2.minAreaRect(c)[0][1])}")                #Daniel        
            ...
            cv2.putText(img,f"Time Elapsed (s): {timer()-start}",(15,self._vid_height-45),self.font,1,(255,255,255),3)              #Daniel
            cv2.putText(img,f"Frame Counter: {frame_id}",(15,self._vid_height-75),self.font,1,(255,255,255),3)                      #Daniel
        ...
        print(f"________________________________________")                  #Daniel
        print(f"Total Time Elapsed: {timedelta(seconds=end-start)}")        #Daniel
        print(f"________________________________________")                  #Daniel